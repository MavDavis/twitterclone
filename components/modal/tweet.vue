<template>
  <div
    class="fixed top-0 left-0 w-screen h-screen bg flex justify-center items-center z-50"
  >
<div class="bg-white w-full h-full fixed top-0 left-0 z-50"  v-if="$store.state.loading">
  <loading/>
    ...
    <div class="xxs:h-96 py-4 h-full bg-white width xxs:rounded-lg overflow-y-scroll">
      ...
      <div class="flex items-center px-4">
        ...
        >
          <img
            v-if="$store.state.userProfile.profileImage.length"
            class="relative w-full h-full rounded-full"
            :src="$store.state.userProfile.profileImage"
            alt=""
          />
        </div>
        <div class="border rounded-full px-2 flex justify-center items-center">
          <span class="text-sm text-dim-500">Everyone</span>
          <i
            @click="toggleTweetAudience()"
            class="fas fa-chevron-down text-sm text-dim-500 ml-2 cursor-pointer"
          ></i>
        </div>
      </div>
      <div class="p-4 ml-12">
        ...
        <textarea
          ...
          rows="4"
          @keypress="$store.commit('ckeckfortweetMessage')"
          v-model="$store.state.tweetMessage"
          placeholder="What's happening?"
        ></textarea>
        <div class="border"></div>
        <div class="flex w-full items-center">
          <div class="w-5/6 py-2 flex justify-start items-center">
            <input
              type="file"
              name="inputTag"
              id="inputTag"
          ...
          <button
            @click="$store.commit('TweetAMessage')"
            class="flex items-center justify-center w-1/6 relative py-1 text-sm rounded-full my-1 text-white cursor-pointer"
            :class="[
              $store.state.youCanTweet
                ? 'bg-dim-500 hover:bg-dim-600 '
                : 'bg-dim-100',
            ]"
          >
            Tweet
